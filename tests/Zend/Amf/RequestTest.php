...
class Zend_Amf_RequestTest extends PHPUnit_Framework_TestCase
{
    ...
    protected function _loadAndParseRequestFromFile($file, $typeLoader = null){
        $request = file_get_contents($file);
        $this->_request->initialize($request);
        
        if($typeLoader == null){
            $typeLoader = new Zend_Amf_TypeMapper();
        }
        $this->_request->setTypeMapper($typeLoader);
        $this->_request->parse();
    }
     ...
     */
    public function testParseThrowsExceptionWithoutTypeMapper()
    {
        $request = new Zend_Amf_Request();
        $request->initialize('bogus data');
        $request->parse();
    }
    ...
    {
        $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/undefAmf3Request.bin');
    ...
    {
        $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/stringAmf3Request.bin');
        // Make sure the encoding type is properly set.
        $this->assertEquals(0x03, $this->_request->getObjectEncoding());
    ...
    {
         $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/arrayAmf3Request.bin');
        // Make sure the encoding type is properly set.
        $this->assertEquals(0x03, $this->_request->getObjectEncoding());
        // Make sure that no headers where recieved
        $this->assertEquals(0 , sizeof($this->_request->getAmfHeaders()));
    ...
    {
        $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/numberAmf3Request.bin');
        // Make sure the encoding type is properly set.
        $this->assertEquals(0x03, $this->_request->getObjectEncoding());
        // Make sure that no headers where recieved
        $this->assertEquals(0 , sizeof($this->_request->getAmfHeaders()));
    ...
    {
        $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/dateAmf3Request.bin');
        // Make sure the encoding type is properly set.
        $this->assertEquals(0x03, $this->_request->getObjectEncoding());
        // Make sure that no headers where recieved
        $this->assertEquals(0 , sizeof($this->_request->getAmfHeaders()));
    ...
    {
         $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/largeIntAmf3Request.bin');
        // Make sure the encoding type is properly set.
        $this->assertEquals(0x03, $this->_request->getObjectEncoding());
        // Make sure that no headers where recieved
        $this->assertEquals(0 , sizeof($this->_request->getAmfHeaders()));
    ...
    {
         $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/boolTrueAmf3Request.bin');
        // Make sure the encoding type is properly set.
        $this->assertEquals(0x03, $this->_request->getObjectEncoding());
        // Make sure that the message body was set after deserialization
        $this->assertEquals(1, sizeof($this->_request->getAmfBodies()));
    ...
    {
         $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/boolFalseAmf3Request.bin');
        // Make sure the encoding type is properly set.
        $this->assertEquals(0x03, $this->_request->getObjectEncoding());
        // Make sure that the message body was set after deserialization
        $this->assertEquals(1, sizeof($this->_request->getAmfBodies()));
    ...
    {
         $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/xmlAmf3Request.bin');
        // Make sure the encoding type is properly set.
        $this->assertEquals(0x03, $this->_request->getObjectEncoding());
        // Make sure that no headers where recieved
        $this->assertEquals(0 , sizeof($this->_request->getAmfHeaders()));
    ...
    {
        $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/byteArrayAmf3Request.bin');
        // Make sure that no headers where recieved
        $this->assertEquals(0 , sizeof($this->_request->getAmfHeaders()));
        // Make sure that the message body was set after deserialization
        $this->assertEquals(1, sizeof($this->_request->getAmfBodies()));
    ...
    {
        $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/stringAmf0Request.bin');
        // Make sure that no headers where recieved
        $this->assertEquals(0 , sizeof($this->_request->getAmfHeaders()));
        // Make sure that the message body was set after deserialization
        $this->assertEquals(1, sizeof($this->_request->getAmfBodies()));
    ...
    {
        $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/objectAmf0Request.bin');
        // Make sure that no headers where recieved
        $this->assertEquals(0 , sizeof($this->_request->getAmfHeaders()));
        // Make sure that the message body was set after deserialization
        $this->assertEquals(1, sizeof($this->_request->getAmfBodies()));
    ...
    {
        $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/objectFirstParamRequest.bin');
        // Make sure that no headers where recieved
        $this->assertEquals(0 , sizeof($this->_request->getAmfHeaders()));
        // Make sure that the message body was set after deserialization
        $this->assertEquals(1, sizeof($this->_request->getAmfBodies()));
    ...
    {
        $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/mixedArrayAmf0Request.bin');
        // Make sure that no headers where recieved
        $this->assertEquals(0 , sizeof($this->_request->getAmfHeaders()));
        // Make sure that the message body was set after deserialization
        $this->assertEquals(1, sizeof($this->_request->getAmfBodies()));
    ...
    {
        $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/numberAmf0Request.bin');
        // Make sure that no headers where recieved
        $this->assertEquals(0 , sizeof($this->_request->getAmfHeaders()));
        // Make sure that the message body was set after deserialization
        $this->assertEquals(1, sizeof($this->_request->getAmfBodies()));
    ...
    {
        $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/dateAmf0Request.bin');
        // Make sure that no headers where recieved
        $this->assertEquals(0 , sizeof($this->_request->getAmfHeaders()));
        // Make sure that the message body was set after deserialization
        $this->assertEquals(1, sizeof($this->_request->getAmfBodies()));
    ...
    {
        $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/intAmf0Request.bin');
        // Make sure that no headers where recieved
        $this->assertEquals(0 , sizeof($this->_request->getAmfHeaders()));
        // Make sure that the message body was set after deserialization
        $this->assertEquals(1, sizeof($this->_request->getAmfBodies()));
    ...
    {
        $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/boolTrueAmf0Request.bin');
        // Make sure that no headers where recieved
        $this->assertEquals(0 , sizeof($this->_request->getAmfHeaders()));
        // Make sure that the message body was set after deserialization
        $this->assertEquals(1, sizeof($this->_request->getAmfBodies()));
...
/**
    ...
    {
        $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/boolFalseAmf0Request.bin');
        // Make sure that no headers where recieved
        $this->assertEquals(0 , sizeof($this->_request->getAmfHeaders()));
        // Make sure that the message body was set after deserialization
        $this->assertEquals(1, sizeof($this->_request->getAmfBodies()));
    ...
    {
        $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/nullAmf0Request.bin');
        // Make sure that no headers where recieved
        $this->assertEquals(0 , sizeof($this->_request->getAmfHeaders()));
        // Make sure that the message body was set after deserialization
        $this->assertEquals(1, sizeof($this->_request->getAmfBodies()));
   ...
   public function testAmf0UndefinedDeserializedToNativePhpNull()
    {
        $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/undefinedAmf0Request.bin');
        // Make sure that no headers where recieved
        $this->assertEquals(0 , sizeof($this->_request->getAmfHeaders()));
        // Make sure that the message body was set after deserialization
        $this->assertEquals(1, sizeof($this->_request->getAmfBodies()));
    ...
    {
        $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/xmlAmf0Request.bin');
        // Make sure that no headers where recieved
        $this->assertEquals(0 , sizeof($this->_request->getAmfHeaders()));
        // Make sure that the message body was set after deserialization
        $this->assertEquals(1, sizeof($this->_request->getAmfBodies()));
    ...
    {
        $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/referenceAmf0Request.bin');
        // Make sure that no headers where recieved
        $this->assertEquals(0 , sizeof($this->_request->getAmfHeaders()));
        // Make sure that the message body was set after deserialization
        $this->assertEquals(1, sizeof($this->_request->getAmfBodies()));
    ...
    {
        $typeLoader = new Zend_Amf_TypeMapper();
        $typeLoader->setClassMap("ContactVO","Contact");
        
        $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/typedObjectAmf0Request.bin', $typeLoader);
        // Make sure that no headers where recieved
        $this->assertEquals(0 , sizeof($this->_request->getAmfHeaders()));
        // Make sure that the message body was set after deserialization
        $this->assertEquals(1, sizeof($this->_request->getAmfBodies()));
    ...
    {
        $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/bogusTypedObjectAmf0Request.bin');
    ...
    {
        $this->_loadAndParseRequestFromFile(dirname(__FILE__) .'/Request/mock/credentialsheaderAmf0.bin');
        // Make sure that no headers where recieved
